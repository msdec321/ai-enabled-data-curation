...
4. **Web console** — [`frontend/`](frontend/): `setup_frontend.py` provisions Cognito (its CloudFront/S3 half is retired — see `decommission_cloudfront.py`). The UI itself deploys by running `frontend/stamp_revision.py frontend/index.html` (stamps the footer's revision date, which the institutional web standards require) and then pushing `index.html`/`app.js`/`config.js`/`uth_logo.svg` to the [autodqa-frontend](https://github.com/uthh-sbmi-ai/autodqa-frontend) GitHub repo, which the institutional Cloudflare Pages serves behind Access.
